...
${global.prefix}kill → Elimina un archivo guardado.`;
  ...
  {
    image: { url: "https://files.catbox.moe/zxwp9c.jpg" }, 
  ...
  try {
    ...
    await sock.sendMessage(chatId, {
      image: { url: imageUrl },
      caption: `${e} Aquí tienes tu Waifu NSFW` }, { quoted: msg });
...
case "modoadmins": {
  try {
    ...
    await sock.sendMessage(chatId, {
      text: `${e} Modo admins *${args[0] === "on" ? "activado" : "desactivado"}* en este grupo.`
    }, { quoted: msg });
        ...
        if (qr && !sentCodeMessage) {
          if (usarPairingCode) {
            ...
            /*await sock.sendMessage(msg.key.remoteJid, {
              ...
              caption: "🔐 *Código generado:*\nAbre WhatsApp\n> Vincular dispositivo y pega el siguiente código:",
              gifPlayback: true
            }, { quoted: msg });*/
            await sock.sendMessage(msg.key.remoteJid, {
...
await sock.sendMessage(msg.key.remoteJid, {
  ...
  caption: `📲 Escanea este código QR desde *WhatsApp > Vincular dispositivo* para conectarte como subbot.`
        ...
        switch (connection) {
          case "open":
            await sock.sendMessage(msg.key.remoteJid, {
              text: `${e} *Conectado exitosamente.*
> 🛠️ Usa los siguientes comandos para comenzar:

${global.prefix}help
${global.prefix}menu

1• Por defecto, el subbot está en *modo privado*, lo que significa que *solo tú puedes usarlo*.

2• Usa el comando: *#menu* (para ver configuraciones y cómo hacer que otras personas puedan usarlo.)

3• Los prefijos por defecto son: *. y #* Si quieres cambiarlos, usa: *#setprefix*

4• Si notas que el subbot *no responde al instante* o tarda mucho *aunque esté conectado*, no te preocupes. Puede ser un fallo temporal.

5• En ese caso, simplemente ejecuta: #delbots para eliminar tu sesión y luego vuelve a conectarte usando: *#serbot* o para code si no quieres qr usa: *#code* o *#sercode.* hasta que se conecte correctamente.

${e} Esto ayuda a establecer una conexión *estable y funcional*.`
            }, { quoted: msg });
...
Usa ${global.prefix}serbot para volver a conectar.`
              ...
              default:
                await sock.sendMessage(msg.key.remoteJid, {
                  text: `⚠️ *Problema de conexión detectado:* ${messageError}
> Intentando reconectar...

_Si seguir en problemas, En ese caso, simplemente ejecuta:_ *#delbots* para eliminar tu sesión y luego vuelve a conectarte usando:
👉🏻 #serbot o para code si no quieres qr usa: #code o #sercode. hasta que se conecte correctamente.`
                }, { quoted: msg });
                break;
            }
            break;
...
case 'tovideo': {
  ...
  if (!quoted) {
    await sock.sendMessage(msg.key.remoteJid, {
      text: `${e} Responde a un sticker para convertirlo a video.`
    }, { quoted: msg });
    break;
  }
...
case 'tourl': {
    ...
    if (!quotedMsg) {
        await m.reply(`${e} *Responde a una imagen, video, sticker, nota de voz o audio para subirlo.*`);
        break;
    }
...
case 'whatmusic': {
    ...
    if (!quotedMsg || (!quotedMsg.audioMessage && !quotedMsg.videoMessage)) {
        await sock.sendMessage(msg.key.remoteJid, {
            text: `${e} Responde a un *audio* (MP3) o *video* (MP4) para identificar la canción.`
        }, { quoted: msg });
        break;
    }
...
case 'whatmusic6': {
    ...
    if (!quotedMsg || (!quotedMsg.audioMessage && !quotedMsg.videoMessage)) {
        await sock.sendMessage(msg.key.remoteJid, {
            text: `${e} Responde a una nota de voz, audio o video para identificar la canción.`
        }, { quoted: msg });
        break;
    }
...
case 'ff2': {
    ...
    if (!audioMsg && !isAudioDoc) {
        await sock.sendMessage(msg.key.remoteJid, {
            text: `${e} Responde a un *audio* o *mp3 dañado* para repararlo.`
        }, { quoted: msg });
        break;
    }
...
case 'notify': {
  try {
    ...
    if (!messageToForward) {
      await sock.sendMessage(chatId, { text: `${e} Debes responder a un mensaje o proporcionar un texto para reenviar.` }, { quoted: msg });
      return;
    }
...
case 'linia': {
  ...
  case 'ff': {
    ...
    if (!quotedMsg || !quotedMsg.videoMessage) {
        await sock.sendMessage(msg.key.remoteJid, {
            text: `${e} Responde a un *video* para optimizarlo para WhatsApp.`
        }, { quoted: msg });
        break;
    }
...
case 'audio': {
    ...
    try {
        if (!text || text.trim() === "") {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `${e} Escribe por favor el nombre de la canción.\nEjemplo: *${global.prefix}play1 Boza Yaya*`
            }, { quoted: msg });
            return;
        }
...
case 'transferir': {
  ...
  if (!amount || amount <= 0) {
    return await sock.sendMessage(msg.key.remoteJid, { text: `${e} Uso correcto: \`${global.prefix}tran <cantidad>\` (cita o menciona al usuario).` }, { quoted: msg });
  }
  ...
  if (!targetJid) {
    return await sock.sendMessage(msg.key.remoteJid, { text: `${e} Debes citar o mencionar al usuario al que quieres transferir.` }, { quoted: msg });
  }
...
case 'gifvideo': {
    try {
        ...
        if (!quoted || !quoted.videoMessage) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `${e} *Responde a un video para convertirlo en estilo GIF largo.*`
            }, { quoted: msg });
            return;
        }
...
case 'gremio': {
    try {
        ...
        if (!rpgData.usuarios || Object.keys(rpgData.usuarios).length === 0) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: "📜 *No hay miembros registrados en el Gremio*\nUsa `" + global.prefix + "rpg <nombre> <edad>` para unirte." 
            }, { quoted: msg });
            return;
        }
        ...
        let listaMiembros = 
`
📋 *Total de miembros registrados en gremio:* ${miembros.length}\n`;
...
case 'infogrupo': {
  ...
  break;
    ...
    if (!text) {
        return sock.sendMessage(msg.key.remoteJid, {
            text: `⚠️ *Uso incorrecto.*\n\n📌 *Ejemplo:* *${global.prefix}tiktokstalk Bad Bunny*`
        }, { quoted: msg });
    }
...
case 'botfoto': {
  ...
  if (!quotedMsg || !quotedMsg.imageMessage) {
    await sock.sendMessage(msg.key.remoteJid, { text: `${e} Debes responder a un mensaje que contenga una imagen para actualizar la foto del bot.` });
    return;
  }
...
case 'botname': {
  ...
  if (!newName) {
    await sock.sendMessage(msg.key.remoteJid, { text: `${e} Debes proporcionar un nuevo nombre para el bot.` });
    return;
  }
...
case 'daradmins': {
  try {
    ...
    if (!targetId) {
      await sock.sendMessage(chatId, { text: `${e} Debes responder a un mensaje o mencionar a un usuario para promoverlo.` }, { quoted: msg });
      return;
    }
...
case 'quitaradmins': {
  try {
    ...
    let targetId = msg.message?.extendedTextMessage?.contextInfo?.participant || (msg.mentionedJid && msg.mentionedJid[0]);
    if (!targetId) {
      await sock.sendMessage(chatId, { text:`${e} Debes responder a un mensaje o mencionar a un usuario para quitarle admin.` }, { quoted: msg });
      return;
    }
...
case 'setfoto': {
  try {
    ...
    if (!msg.message?.extendedTextMessage?.contextInfo?.quotedMessage ||
      ...
      await sock.sendMessage(chatId, { text: `${e} *Debes responder a un mensaje que contenga una imagen para establecerla como foto de grupo.*` }, { quoted: msg });
      return;
    }
...
case 'personalidad': {
  try {
    ...
    if (!userId) {
      return sock.sendMessage(
        msg.key.remoteJid,
        { text: `${e} *Debes mencionar a un usuario o responder a su mensaje para analizar su personalidad.*` },
        { quoted: msg }
      );
    }
...
case 'batallauser': {
  try {
    ...
    if (!rpgData.usuarios[userId]) {
      return sock.sendMessage(msg.key.remoteJid, {
        text: `❌ *No tienes una cuenta en el gremio. Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.*`
      }, { quoted: msg });
    }
    let usuario = rpgData.usuarios[userId];
...
case 'verper': { 
    try { 
        ...
        if (!rpgData.usuarios[userId]) { 
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No estás registrado en el gremio.*\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
            return; 
        }
        ...
        let mensaje = `🎭 *Tus Personajes Comprados* 🎭\n\n`;
    ...
    try {
        ...
        if (!fs.existsSync(rpgFile)) {
            await sock.sendMessage(
                msg.key.remoteJid,
                {
                    text: `❌ *No tienes una cuenta en el gremio.*\n\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.`
                },
                { quoted: msg }
            );
            return;
...
case 'nivelper': {
    try {
        ...
        if (!rpgData.usuarios[msg.key.participant]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No tienes cuenta en el bot.*\n📌 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
            return;
        }
...
case 'topuser': {
    try {
        ...
        let ranking = `🏆 *Ranking de Jugadores del Gremio`;
        let mentions = [];
...
case 'rpg': { 
    try { 
        if (args.length < 2) { 
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `⚠️ *Uso incorrecto.*\nEjemplo: \`${global.prefix}rpg edi 18\`` 
            }, { quoted: msg });
            return; 
        }
        ...
        if (rpgData.usuarios[userId]) { 
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `⚠️ *Ya estás registrado en el gremio.*\n\n📜 Usa \`${global.prefix}menurpg\` para ver tus opciones.` 
            }, { quoted: msg });
            return; 
        }

        await sock.sendMessage(msg.key.remoteJid, { react: { text: "⏳", key: msg.key } }); 
        let registroMensaje = await sock.sendMessage(msg.key.remoteJid, { text: `📝 *Registrando en el Gremio*` }, { quoted: msg }); 
...
case 'vermascotas': {  
    try {  
        ...
        mensaje += `🚀 **¡Sigue entrenando a tus mascotas en el Gremio!** 🏆`;  
...
case 'deleteuser': {
    try {
        ...
        if (!rpgData.usuarios[userId]) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `❌ *El usuario @${text} no tiene una cuenta registrada en el gremio.*`,
                mentions: [userId]
            }, { quoted: msg });
            return;
        }
        ...
        await sock.sendMessage(msg.key.remoteJid, {
            text: `🗑️ *La cuenta de @${text} ha sido eliminada exitosamente del gremio.*\n\n` +
...
case 'deleterpg': {
    try {
        ...
        if (!rpgData.usuarios[userId]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No tienes un registro en el gremio.*\n\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
            return;
        }
...
case 'tiendamascotas': {
    try {
        ...
        let mensaje = `🏪 *Tienda de Mascotas* 🏪\n\n`;
        ...
        mensaje += `🔹 Usa *${global.prefix}mascota <número>* para cambiar tu mascota principal.\n\n`;
        mensaje += `🚀 **¡Colecciona y entrena las mejores mascotas en el Gremio!** 🏆`;
...
case "stiker":
    try {
        ...
        let content = msg.message[type];

        // Captura el caption si existe
        let caption = content?.caption || "";
        let mediaMessage = (type === "imageMessage" || type === "videoMessage") ? content : null;

        // Verifica si también es una respuesta a media
        let quoted = msg.message.extendedTextMessage?.contextInfo?.quotedMessage;
        let quotedMedia = quoted?.imageMessage || quoted?.videoMessage;

        // Verifica si viene con media directa o citada
        if (!mediaMessage && !quotedMedia) {
            const xds = `👾 *Uso correcto:*\nEnvía o responde a una imagen/video con "${global.prefix}s" para convertirlo en sticker.\n\nEjemplo: Responde a una foto con ${global.prefix}s o envíala con el pie de foto "${global.prefix}s"`;
            await sock.sendMessage(msg.key.remoteJid, {
                text: xds
            }, { quoted: msg });
            return;
        ...
        let mediaType = mediaMessage ? (type === "imageMessage" ? "image" : "video") : (quoted.imageMessage ? "image" : "video");
        let mediaContent = mediaMessage || quoted[`${mediaType}Message`];
