...
const handler = async (msg, { conn }) => {
  ...
  if (!quotedInfo) {
    return await conn.sendMessage2(msg.key.remoteJid, {
      text: `${e} *Error:* Debes responder a un mensaje de *ver una sola vez* (imagen, video o audio) para poder verlo nuevamente.`
    }, msg );
  }
  ...
  if (quotedInfo.imageMessage?.viewOnce) {
    ...
    return await conn.sendMessage2(msg.key.remoteJid, {
      text: `${e} *Error:* Solo puedes usar este comando en mensajes de *ver una sola vez*.`
    }, msg );
  }
  ...
  if (!mediaStream || mediaStream.length === 0) {
    return await conn.sendMessage2(msg.key.remoteJid, {
      text: `${e} *Error:* No se pudo descargar el archivo. Intenta de nuevo.`
    }, msg );
  }
